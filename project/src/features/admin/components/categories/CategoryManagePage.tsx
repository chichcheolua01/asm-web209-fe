  ...
  return (
    <>
      ...
      <div className="relative overflow-x-auto shadow-2xl sm:rounded-lg mt-10">
        <div className="p-3">
          <Link to={"create"}>
            <button className="py-2 px-4 bg-gray-600 text-white font-semibold rounded-lg hover:bg-gray-700 shadow-mdfocus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75">
              Add category
            </button>
          </Link>
        </div>
            ...
            {data &&
              data.categories.map((cate, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-3">{cate.name}</td>
                  <td className="px-6 py-3">
                    <Link to={`update/${cate._id}`}>
